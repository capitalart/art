# FULL CODE STACK (THU-31-JULY-2025-02-11-PM)
...
def analyze_artwork_route(aspect, filename):
    ...
    # Locate the source image in either unanalysed or processed folders (for re-analysis)
    ...
    try:
        ...
        # The analysis script returns the full listing dictionary
        analysis_result = _run_ai_analysis(src_path, provider)
        
        # Extract the seo_folder from the returned path
        processed_folder_path = Path(analysis_result.get("processed_folder", ""))
        seo_folder = processed_folder_path.name
        new_filename = analysis_result.get("seo_filename")

        if not seo_folder or not new_filename:
            raise RuntimeError("Analysis script did not return a valid folder or filename.")

        _write_analysis_status("generating", 60, filename, status="analyzing")
        # Immediately trigger composite generation for the new folder
        ...
        if "XMLHttpRequest" in request.headers.get("X-Requested-With", ""):
            return jsonify({"success": False, "error": str(exc)}), 500
        # Redirect back to the artworks page on failure
        return redirect(url_for("artwork.artworks"))
...
def analyze_upload(base):
    ...
    try:
        # Run analysis and get the results directly
        ...
        # Use the main composite generation function for consistency
        _generate_composites(seo_folder, uuid.uuid4().hex)
    ...
    # Redirect using the filename from the analysis result
    return redirect(url_for("artwork.edit_listing", aspect=qc.get("aspect_ratio", ""), filename=new_filename))
...
def generate_seo_filename(raw_title: str, assigned_sku: str) -> str:
    ...
    Format: [core-title-slug]-[seo-keywords]-artwork-by-robin-custance-[sku].jpg
    """
    ...
    # 2. Extract the core art-specific words by removing all generic phrases
    ...
    # Further cleanup and create the base slug from what's left
    clean_title = re.sub(r'\s+', ' ', clean_title).strip()
    base_slug = slugify(clean_title if clean_title else "untitled-artwork")

    # 3. Build the filename by adding SEO keywords until the length limit is approached
    final_slug_parts = [base_slug]
    
    # Calculate the length of the required final parts of the filename
    fixed_suffix_len = len(suffix) + len(assigned_sku) + 5  # for hyphens and .jpg
    
    current_len = len(base_slug) + fixed_suffix_len
    
    for keyword in seo_keywords_to_add:
        # Check if adding the next keyword would exceed the 70-character limit
        if current_len + len(keyword) + 1 > 70:
            break
        final_slug_parts.append(keyword)
        current_len += len(keyword) + 1
        
    # 4. Assemble the final filename string
...
def add_to_mockup_queue(artwork_path: str):
    ...
    try:
        if queue_file.exists():
            ...
            queue_file.write_text(json.dumps([artwork_path], indent=2), encoding="utf-8")
...
def analyze_single(image_path: Path):
    ...
    try:
        ...
        # Step 5: Generate the SEO folder name and filename
        seo_name = slugify(raw_title) # Use the full, cleaned title for the folder name
        ...
        # Step 10: Add the processed artwork to the mockup generation queue
        ...
        # Step 11: Clean up temporary optimized file
...
-   **primary\_colour, secondary\_colour**:
  ...
  <div class="review-artwork-grid row">
    ...
    <div class="col col-6 edit-listing-col">
      ...
      <form role="form" id="edit-form" method="POST" autocomplete="off">
        ...
        {# AI Rewording Tool #}
        <div class="artwork-info-card" id="generic-text-reworder" data-generic-text="{{ artwork.generic_text|e }}">
          ...
          {% for info in entries %}
            <table class="openai-analysis-table">
              <tbody>
                ...
                {% if info.api_response %}
                  <tr><th>API Response</th><td>{{ info.api_response }}</td></tr>
                {% endif %}
                {% if info.naming_method %}
                  <tr><th>Naming Method</th><td>{{ info.naming_method }}{% if info.used_fallback_naming %} (fallback){% endif %}</td></tr>
                {% endif %}
              </tbody>
            </table>
          {% endfor %}
      ...
      {% endif %}
    </div> <!-- end of .edit-listing-col -->

  </div> <!-- end of .review-artwork-grid -->
...
<div class="finalised-grid" data-view-key="lockedView">
  ...
  <div class="button-row">
    <a class="art-btn disabled" aria-disabled="true">Edit</a>
    <form method="post" action="{{ url_for('artwork.unlock_listing', aspect=art.aspect, filename=art.seo_folder ~ '.jpg') }}">
      <button type="submit" class="art-btn">Unlock</button>
    </form>
    <form method="post" action="{{ url_for('artwork.delete_finalised', aspect=art.aspect, filename=art.seo_folder ~ '.jpg') }}" class="locked-delete-form">
      <input type="hidden" name="confirm" value="">
      <button type="submit" class="art-btn delete">Delete</button>
    </form>
  </div>
...
# AUTHOR: Robbie Mode™ Patch - 2025-07-30 (Updated with hidden div marker check)
# ======================================================================================
...
def test_analyze_api_html_response(client):
    """
    Ensure that /analyze/openai/<filename> returns valid HTML content after redirect.
    The system no longer returns JSON — it redirects to the listing editor page.
    """
    ...
    # --- [ 2.1: Ensure test image exists in unanalysed folder ] ---
    ...
    # --- [ 2.2: POST to /analyze route and follow redirect to edit page ] ---
...
def teardown_module(module):
    """
    Cleanup unanalysed-artwork test folders after test run.
    Deletes any folders starting with known test prefixes.
    """
