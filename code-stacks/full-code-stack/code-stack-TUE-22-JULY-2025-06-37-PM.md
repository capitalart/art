# FULL CODE STACK (TUE-22-JULY-2025-06-37-PM)
...
# Additional logger for Google API call details
...
if not google_logger.handlers:
    google_handler = logging.FileHandler(google_log_path, encoding="utf-8")
    google_handler.setFormatter(logging.Formatter("%(message)s"))
    google_logger.addHandler(google_handler)
...
def generate_ai_listing(
    ...
    try:
        ...
        logger.error("Failed to read image for %s: %s", provider, e)
    ...
    if provider == "google":
        try:
            ...
            if verbose:
                print(f"- Sending to Google Vision API (model={model_name}, temp=0.92)...")
            parts = [
                prompt,
            ...
            google_logger.info(json.dumps({"provider": "google", "prompt": parts[0], "response": content[:500]}))
            if verbose:
                print(f"- Google response received (length {len(content)} chars). Parsing...")
            try:
                return json.loads(content), True, content, "ok", ""
            except Exception:
                fallback = parse_text_fallback(content)
                return fallback, False, content, "fallback", "Google response not valid JSON"
        except Exception as exc:  # noqa: BLE001
            ...
            return (
                {
                    "fallback_text": "Google API failed, no listing generated.",
                    "error": str(exc),
                    "model_used": config.GOOGLE_GEMINI_PRO_VISION_MODEL_NAME,
...
def analyze_single(
    ...
    try:
        ...
        if not was_json:
            ...
            (google_logger if provider == "google" else openai_logger).info(json.dumps(safe_entry))
