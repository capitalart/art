# FULL CODE STACK (TUE-22-JULY-2025-07-15-PM)
...
def _run_ai_analysis(img_path: Path, provider: str) -> dict:
    """Run the AI analysis by calling the appropriate external script."""
    logger = logging.getLogger(__name__)
    logger.info("Running analysis for %s with %s", img_path, provider)

    if provider == "openai":
        if not config.OPENAI_API_KEY:
            raise RuntimeError("OPENAI_API_KEY not set")
        script_path = utils.ANALYZE_SCRIPT_PATH
        cmd = ["python3", str(script_path), str(img_path), "--provider", "openai"]
    elif provider == "google":
        if not os.getenv("GOOGLE_API_KEY"):
            raise RuntimeError("GOOGLE_API_KEY not set")
        # Define the path to the new script
        google_script_path = config.SCRIPTS_DIR / "analyze_artwork_google.py"
        cmd = ["python3", str(google_script_path), str(img_path)]
    else:
    ...
    try:
        ...
        seo_folder = utils.find_seo_folder_from_filename("", img_path.name)
        listing_path = PROCESSED_ROOT / seo_folder / f"{seo_folder}-listing.json"

        if not listing_path.exists():
            raise RuntimeError(
                f"Analysis script ran, but listing JSON was not found at {listing_path}"
            )

        return utils.load_json_file_safe(listing_path)

    except subprocess.CalledProcessError as e:
        logger.error("Analysis script for %s failed with exit code %s", provider, e.returncode)
        logger.error("STDERR:\n%s", e.stderr)
        raise RuntimeError(f"Analysis failed: {e.stderr.strip()}")
    except Exception as e:
        logger.error("Failed to execute analysis script for %s: %s", provider, e)
        raise
...
def regenerate_one_mockup(seo_folder: str, slot_idx: int) -> bool:
    ...
    mockup_files = list((MOCKUPS_INPUT_DIR / category).glob("*.png"))
    if not mockup_files:
        return False
...
def swap_one_mockup(seo_folder: str, slot_idx: int, new_category: str) -> bool:
    ...
    if slot_idx < 0 or slot_idx >= len(mockups):
        return False
    mockup_files = list((MOCKUPS_INPUT_DIR / new_category).glob("*.png"))
    if not mockup_files:
        return False
    new_mockup = random.choice(mockup_files)
    aspect = data.get("aspect_ratio")
...
def parse_args():
    ...
    parser.add_argument(
        "--provider", choices=["openai", "google"], default="openai", help="AI provider to use (default: openai)"
    )
...
def main() -> None:
    ...
    if single_path:
        ...
        entry = analyze_single(single_path, system_prompt, feedback_text, statuses, 1, 1, verbose, provider)
        if entry:
...
def analyze_with_google(image_path: Path):
    ...
    try:
        ...
        google_logger.info(json.dumps(log_entry))
...
def main():
    ...
    for img_path in queue[:]:  # Copy list, in case we mutate queue
        ...
        mockups_cat_dir = config.MOCKUPS_INPUT_DIR / f"{aspect}-categorised"
        coords_dir = config.COORDS_DIR / aspect
...
.mockup-card { background: #fff; box-shadow: 0 2px 10px #0001; padding: 11px 7px; text-align: center;border: 1px solid var(--card-border);}

.mockup-thumb-img { width: 100%; border: 1px solid #eee; box-shadow: 0 1px 6px rgba(0,0,0,0.09); background: none; cursor: pointer; transition: box-shadow 0.15s;}
...
@media (max-width: 800px) {
  ...
  .mockup-preview-grid { flex-direction: column; gap: 1em; }
  .mockup-card { width: 100%; }
  .mini-mockup-grid { grid-template-columns: 1fr 1fr 1fr;}
}
...
@media (max-width: 1300px) {
  .mockup-thumb-img { max-width: 260px; }
}
@media (max-width: 900px) {
  .mockup-thumb-img { max-width: 180px; }
}
@media (max-width: 600px) {
  .mockup-thumb-img { max-width: 100%; }
}
  ...
  fetch(`/analyze-${provider}/${encodeURIComponent(filename)}`, {
    ...
    .then(d => {
      ...
      if (d.success) {
        ...
        if (window.AnalysisModal) window.AnalysisModal.open({message: d.error || 'Analysis failed'});
      }
    })
    .catch(err => { hideOverlay(card); setStatus(card, false); if (window.AnalysisModal) { window.AnalysisModal.open({message: err.message || 'Analysis failed'}); } });
}
