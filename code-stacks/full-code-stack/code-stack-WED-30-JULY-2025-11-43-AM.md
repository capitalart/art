# FULL CODE STACK (WED-30-JULY-2025-11-43-AM)
...
from ..utils.csv_utils import generate_single_csv_row_data
...
# === [ analyze-routes-py-7: Additional Artwork Actions ] ===

@router.post("/mark-ready-export/{artwork_id}", name="mark_artwork_ready_for_export")
async def mark_artwork_ready_for_export_route(
    artwork_id: int, request: Request, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_active_user)
):
    """Marks a finalized artwork as ready for export."""
    logger.info(f"User '{current_user.username}' marking artwork ID {artwork_id} as ready for export.")
    artwork_db = _get_artwork_or_404(artwork_id, db, current_user)  # Corrected: No await
    analyze_page_url = str(request.url_for('analyze_artworks_page'))

    if artwork_db.status != "finalized":
        error_msg = (
            f"Artwork ID {artwork_db.id} must be finalized before marking ready for export. Current status: {artwork_db.status}"
        )
        return RedirectResponse(
...
# === [ analyze-routes-py-8: Full Listing Preview Page ] ===
...
async def full_listing_preview_page(
    ...
    """Displays a comprehensive Etsy/Nembol preview for a single artwork."""
    ...
    try:
        if ETSY_MASTER_TEMPLATE_PATH.is_file():
            master_template = ETSY_MASTER_TEMPLATE_PATH.read_text(encoding="utf-8")
            master_template = clean_listing_text(master_template)

            if artwork.aspect_ratio_str:
                m = re.match(r"(\d+:\d+)", artwork.aspect_ratio_str)
                if m:
                    aspect_ratio_key = m.group(1)
            ...
            if artwork.ai_profile_used and "aboriginal" in artwork.ai_profile_used.lower():
                dot_painting_block = get_dot_painting_history_block()

            assembled_description = master_template.replace("{AI_GENERATED_TITLE}", artwork.generated_title or "")
            assembled_description = assembled_description.replace("{AI_GENERATED_FULL_DESCRIPTION_CONTENT}", artwork.generated_description or "")
            assembled_description = assembled_description.replace("{ASPECT_RATIO_DETAILS_BLOCK}", aspect_details_block)
            assembled_description = assembled_description.replace("{CONDITIONAL_DOT_PAINTING_HISTORY_BLOCK}", dot_painting_block)
            assembled_description = assembled_description.replace("https://www.etsy.com/au/shop/RobinCustance", settings.ETSY_SHOP_URL or "#")
    ...
    nembol_data = generate_single_csv_row_data(db, artwork.id)
...
async def admin_artwork_detail_route(
    ...
    """(Admin) Displays a detailed view of an artwork record."""
    if not current_user.is_superuser:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin access required.")

    artwork = _get_artwork_or_404(artwork_id, db, current_user)  # Corrected: No await
...
async def get_artworks_for_analysis_gallery_api(
    ...
    """API endpoint to get a list of artworks for the gallery display."""
    gallery_statuses = ["qc_passed", "analyzed_pending_acceptance", "analysis_rejected", "finalized", "ready_for_export"]
    artworks_db = crud.get_artworks_by_statuses(db, statuses=gallery_statuses, owner_id=None if current_user.is_superuser else current_user.id)
...
async def update_artwork_status_api_route_analyze(
    artwork_id: int, new_status: str = Form(...), db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_active_user)
) -> JSONResponse:
    """API endpoint to update the status of an artwork."""
    artwork = _get_artwork_or_404(artwork_id, db, current_user)  # Corrected: No await
    valid_statuses = [
        "uploaded_pending_qc",
        "qc_passed",
