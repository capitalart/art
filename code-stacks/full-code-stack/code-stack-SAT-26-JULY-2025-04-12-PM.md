# FULL CODE STACK (SAT-26-JULY-2025-04-12-PM)
...
from routes.mockup_routes import bp as mockup_bp
...
_CRITICAL_FOLDERS = [
    ...
    SELLBRITE_DIR, SIGNED_DIR, MOCKUPS_INPUT_DIR, SIGNATURES_DIR,
    GENERIC_TEXTS_DIR, COORDS_DIR, SCRIPTS_DIR, SETTINGS_DIR, LOGS_DIR,
    CODEX_LOGS_DIR, DATA_DIR, STATIC_DIR, TEMPLATES_DIR,
]
...
# -----------------------------------------------------------------------------
# 11. CRITICAL FILE CREATION
# -----------------------------------------------------------------------------
_CRITICAL_FILES = {
    LOGS_DIR / "session_registry.json": "{}",
    LOGS_DIR / "session_registry.tmp": "{}",
}
for file_path, default_content in _CRITICAL_FILES.items():
    try:
        if not file_path.exists():
            file_path.write_text(default_content, encoding="utf-8")
    except Exception as exc:
        raise RuntimeError(f"Could not create required file {file_path}: {exc}")
...
---
# routes/artwork_routes.py
"""Artwork-related Flask routes for the core artwork lifecycle."""
...
def _process_upload_file(file_storage, dest_folder):
    ...
    qc_data = {
        ...
        "filesize_bytes": len(data), "aspect_ratio": aspect,
    ...
    if request.method == "POST":
        ...
        for f in files:
            try:
                ...
                logging.getLogger(__name__).error("Upload processing failed for %s: %s", f.filename, exc)
                res = {"original": f.filename, "success": False, "error": str(exc)}
            status = "success" if res.get("success") else "fail"
            log_action(
                "upload", res.get("original", f.filename), user,
                res.get("error", "uploaded"), status=status,
...
def edit_listing(aspect, filename):
    ...
    if request.method == "POST":
        ...
        form_data = {
            ...
            "price": request.form.get("price", data.get("price", "18.27")).strip(),
            "sku": sku_val,
        ...
        if action == "delete":
            ...
            shutil.rmtree(config.PROCESSED_ROOT / f"{seo_folder}-THUMBS", ignore_errors=True)
            try:
                os.remove(config.UNANALYSED_ROOT / aspect / filename)
            except Exception:
                pass
        ...
        if errors:
            # Re-populate form with user's invalid data to allow correction
            pass # This logic is complex and needs careful implementation
    ...
    for idx, mp in enumerate(data.get("mockups", [])):
        ...
        thumb_path = thumb_dir / out.name
        
        mockups.append({
            "path": out, "category": cat, "exists": out.exists(), 
            "thumb_path": thumb_path, "thumb_exists": thumb_path.exists(), "index": idx
        })
    
    return render_template(
        "edit_listing.html", artwork=artwork, aspect=aspect, filename=filename,
        seo_folder=seo_folder, mockups=mockups, menu=utils.get_menu(), errors=None,
        colour_options=get_allowed_colours(),
        categories=utils.get_mockup_categories(config.MOCKUPS_CATEGORISED_DIR / f"{aspect}-categorised"),
        finalised=finalised, locked=data.get("locked", False), editable=not data.get("locked", False),
        openai_analysis=openai_info, cache_ts=int(time.time()),
    )
...
def finalise_artwork(aspect, filename):
    ...
    try:
        ...
        (final_dir / "finalised.txt").write_text(datetime.datetime.now().isoformat(), encoding="utf-8")
...
_PROCESSED_REL = config.PROCESSED_ROOT.relative_to(config.BASE_DIR).as_posix()
_FINALISED_REL = config.FINALISED_ROOT.relative_to(config.BASE_DIR).as_posix()
_LOCKED_REL = config.ARTWORK_VAULT_ROOT.relative_to(config.BASE_DIR).as_posix()

@bp.route(f"/static/{_PROCESSED_REL}/<path:path>")
def processed_image(path):
    """Serve artwork images from processed folders, including subdirectories."""
    full_path = config.PROCESSED_ROOT / path
    final_path = config.FINALISED_ROOT / path
    ...
    if final_path.exists():
        ...
        current_app.logger.error("Image not found: %s", path)
        abort(404)

    return send_from_directory(directory, filename)

@bp.route(f"/static/{_FINALISED_REL}/<path:path>")
def finalised_image(path):
    """Serve images strictly from the finalised-artwork folder."""
    directory = config.FINALISED_ROOT / Path(path).parent
    filename = Path(path).name
    return send_from_directory(directory, filename)

@bp.route(f"/static/{_LOCKED_REL}/<path:path>")
def locked_image(path):
    """Serve images from the locked artwork vault."""
    directory = config.ARTWORK_VAULT_ROOT / Path(path).parent
    filename = Path(path).name
    return send_from_directory(directory, filename)

@bp.route("/artwork-img/<aspect>/<filename>")
def artwork_image(aspect, filename):
    folder = config.UNANALYSED_ROOT / aspect
...
# FIX: Add the missing _run_ai_analysis function
def _run_ai_analysis(image_path: Path, provider: str):
    """
    Wrapper to run AI analysis on a single image from a web route.
    """
    system_prompt = aa.read_onboarding_prompt()
    # We need a list to pass to analyze_single for its status reporting
    statuses = []
    # Call the main analysis function from the script
    artwork_entry = aa.analyze_single(
        ...
        idx=1,
        total=1,
        verbose=False, # Don't print verbose output in web context
        provider=provider,
    )
    # Check the status list to see if it succeeded
    if not statuses or not statuses[0].get("success"):
        error_message = statuses[0].get("error", "Unknown analysis error")
...
def run_generator():
    """Runs the coordinate generator script and streams its output in real-time."""
...
bp = Blueprint("mockup", __name__, url_prefix="/mockups")
...
def regenerate():
    ...
    if 0 <= slot_idx < len(slots):
        ...
        session["slots"] = slots
    return redirect(url_for("mockup.select"))
...
def composites_preview():
    ...
    if latest:
        return redirect(url_for("mockup.composites_specific", seo_folder=latest))
...
def regenerate_composite(seo_folder, slot_index):
    ...
    return redirect(url_for("mockup.composites_specific", seo_folder=seo_folder))
...
def review_swap_mockup(seo_folder, slot_idx):
            ...
            try:
                ...
                if composite_name:
                     img_url = url_for(
                        ...
                        path=f"{seo_folder}/thumbnails/{composite_name}"
                     )
    ...
    flash("Could not locate the correct artwork for editing after swap.", "warning")
    return redirect(url_for("artwork.finalised_gallery"))
...
bp = Blueprint("utility", __name__, url_prefix="/utility")
...
def analysis_status():
    ...
    data = load_json_file_safe(config.ANALYSIS_STATUS_FILE)
    if not data:
...
def create_thumbnail(source_path: Path, thumb_dir: Path, size: tuple[int, int]):
    """Creates a resized thumbnail for a given image."""
    thumb_dir.mkdir(parents=True, exist_ok=True)
    thumb_path = thumb_dir / source_path.name
    try:
        with Image.open(source_path) as img:
        ...
        print(f"  -> ⚠️ Could not create thumbnail for {source_path.name}: {e}", flush=True)
...
def pick_mockups(aspect: str, max_count: int = 9) -> list:
    """Select mockup images for the given aspect from categorized folders."""
    mockup_root = config.MOCKUPS_CATEGORISED_DIR / f"{aspect}-categorised"
    if not mockup_root.exists():
        logging.error(f"No categorized mockup folder found for aspect {aspect}")
        return []
    # Find all available mockup PNGs across all categories for this aspect
    all_mockups = list(mockup_root.glob("**/*.png"))
    if not all_mockups:
        logging.warning(f"No PNG mockups found in any category for aspect {aspect}")
        return []
    # Shuffle and select the required number
    random.shuffle(all_mockups)
    selection = [str(p.relative_to(config.MOCKUPS_INPUT_DIR)) for p in all_mockups[:max_count]]
    logging.info(f"Selected {len(selection)} mockups from {mockup_root.name} for {aspect}")
    return selection
...
def regenerate_one_mockup(seo_folder: str, slot_idx: int) -> bool:
    ...
    try:
        ...
        dst = [[c[i]['x'], c[i]['y']] for i in range(4)]
        ...
        # BUG FIX: Generate a thumbnail for the new composite
        thumb_dir = folder / "thumbnails"
        create_thumbnail(output_path, thumb_dir, (config.THUMB_WIDTH, config.THUMB_HEIGHT))
...
def swap_one_mockup(seo_folder: str, slot_idx: int, new_category: str) -> bool:
    ...
    try:
        ...
        # Ensure dst_points are in the correct order for getPerspectiveTransform
        dst = [[c[i]['x'], c[i]['y']] for i in range(4)]
...
def validate_listing_fields(form_data: dict, generic_text: str) -> list[str]:
    """Validate form data fields and return list of error messages."""
    errors = []
    # Title validation
    if not form_data.get("title") or len(form_data.get("title")) < 5:
        errors.append("Title must be at least 5 characters long.")
    # Description validation
    desc = form_data.get("description", "")
    if not desc or len(desc) < 20:
        errors.append("Description must be at least 20 characters long.")
    # Generic text check in description
    if generic_text and generic_text not in desc:
        errors.append("The standard generic text block is missing from the description.")
    # Tags validation
    if not form_data.get("tags") or len(form_data.get("tags")) < 3:
        errors.append("Please provide at least 3 tags.")
    # Materials validation
    if not form_data.get("materials") or len(form_data.get("materials")) < 2:
        errors.append("Please provide at least 2 materials.")
    # Colour validation
    if not form_data.get("primary_colour"):
        errors.append("Primary colour cannot be empty.")
    # Price validation
    try:
...
def _write_status(status_file: Path | None, step: str, percent: int, file: str | None = None, error: str | None = None):
    """Writes progress to the status file for the UI to poll."""
    if not status_file:
        return
...
def analyze_single(
    ...
    try:
        ...
        _write_status(status_file, "Optimizing image for AI", 25, image_path.name)
        ...
        log_entry = {
            "file": str(image_path), "user_id": USER_ID, "opt_image": str(opt_img),
            "size_bytes": size_bytes, "size_mb": round(size_bytes / 1024 / 1024, 2),
            "dimensions": dimensions, "time_sent": _dt.datetime.now(_dt.timezone.utc).isoformat(),
            "provider": provider,
        }
        start_ts = _dt.datetime.now(_dt.timezone.utc)
        analysis_entry = {
            "original_file": str(image_path), "user_id": USER_ID, "optimized_file": str(opt_img),
            "size_bytes": size_bytes, "size_mb": round(size_bytes / 1024 / 1024, 2),
            "dimensions": dimensions, "time_sent": start_ts.isoformat(), "provider": provider,
        }

        _write_status(status_file, "Calling OpenAI API", 40, image_path.name)
        ...
        if not was_json:
            ...
            log_entry.update({"time_responded": end_ts.isoformat(), "duration_sec": (end_ts - start_ts).total_seconds()})
            safe_entry = sanitize_blob_data(log_entry)
            openai_logger.info(json.dumps(safe_entry))
            analysis_entry.update({"time_responded": end_ts.isoformat(), "duration_sec": (end_ts - start_ts).total_seconds(), "status": "fail", "api_response": f"{err_type}: {err_msg}"})
            status["error"] = f"{err_type}: {err_msg}"
            raise RuntimeError(err_msg)

        log_entry.update({"status": "success"})
        call_end_ts = _dt.datetime.now(_dt.timezone.utc)
        analysis_entry.update({"time_responded": call_end_ts.isoformat(), "duration_sec": (call_end_ts - start_ts).total_seconds(), "status": "success", "api_response": "ok"})
        
        _write_status(status_file, "Parsing AI response", 70, image_path.name)
        seo_name, used_fallback_naming, naming_method = extract_seo_filename(ai_listing, raw_response, fallback_base)
        log_entry.update({"used_fallback_naming": used_fallback_naming, "naming_method": naming_method})
        analysis_entry.update({"used_fallback_naming": used_fallback_naming, "naming_method": naming_method})

        if verbose:
            print("- Parsed JSON OK, writing outputs...")

        _write_status(status_file, "Saving files & detecting colours", 85, image_path.name)
        ...
        end_ts = _dt.datetime.now(_dt.timezone.utc)
        log_entry.update({"time_responded": end_ts.isoformat(), "duration_sec": (end_ts - start_ts).total_seconds(), "seo_filename": f"{seo_name}.jpg", "processed_folder": str(folder_path)})
        safe_entry = sanitize_blob_data(log_entry)
        openai_logger.info(json.dumps(safe_entry))

        desc = ai_listing.get("description", "") if isinstance(ai_listing, dict) else ""
        if desc and generic_text and not desc.endswith(generic_text.strip()):
            desc = desc.rstrip() + "\n\n" + generic_text.strip()
        elif not desc:
        ...
        artwork_entry = {
            "filename": image_path.name, "aspect_ratio": aspect, "mockups": mockups,
            "generic_text": generic_text, "ai_listing": ai_listing, "seo_name": seo_name,
            "used_fallback_naming": used_fallback_naming, "main_jpg_path": main_jpg,
            "orig_jpg_path": orig_jpg, "thumb_jpg_path": thumb_jpg,
            "processed_folder": str(folder_path), "primary_colour": primary_colour,
            "secondary_colour": secondary_colour, "tags": tags, "materials": materials, "sku": existing_sku,
        }
        ...
        if verbose:
            print(f"- Error: {e}")
        _write_status(status_file, "Error", 100, image_path.name, error=str(e))
        raise
...
def parse_args():
    ...
    parser.add_argument(
        "--status-file",
        type=Path,
        help="Path to the JSON status file for progress updates.",
    )
    ...
    if single_path:
        ...
        try:
            entry = analyze_single(
                single_path, system_prompt, feedback_text, statuses, status_file, 1, 1, verbose, provider
            )
            if json_output:
        ...
        for idx, img_path in enumerate(sorted(all_images), 1):
            ...
            entry = analyze_single(img_path, system_prompt, feedback_text, statuses, status_file, idx, total, verbose, provider)
            if entry:
                results.append(entry)
...
def main():
    ...
    for img_path_str in queue[:]:
        ...
        # ** UPDATED: Create a subfolder for thumbnails **
        thumb_dir = folder / "thumbnails"
        thumb_dir.mkdir(exist_ok=True)
        
        all_categories = [d for d in mockups_cat_dir.iterdir() if d.is_dir()]
        if not all_categories:
            print(f"⚠️ No category folders found for aspect {aspect}")
            remove_from_queue(str(img_path), QUEUE_FILE)
            continue
            
        random.shuffle(all_categories)
        selected_categories = all_categories[:9]
        print(f"   - Found {len(all_categories)} categories. Selected {len(selected_categories)} to generate mockups from.")

        for category_dir in selected_categories:
            png_mockups = list(category_dir.glob("*.png"))
            if not png_mockups:
                print(f"   - ⚠️ No mockups in selected category: {category_dir.name}")
                continue
            ...
            print(f"   - Mockup & thumbnail created: {output_filename} ({category_dir.name})")
...
def detect_corner_points(pil_image: Image.Image):
    """Detects 4 corner points of a transparent region from a PIL Image."""
    # Convert PIL RGBA image to an OpenCV BGRA NumPy array
    image_np = np.array(pil_image)
    if image_np.shape[2] != 4: return None
    ...
    for aspect_dir in sorted(mockup_root.iterdir()):
        ...
        for category_dir in sorted(aspect_dir.iterdir()):
            ...
            for mockup_file in sorted(category_dir.glob("*.png")):
                ...
                try:
                    # Open with Pillow, which handles sRGB profile issues
                    with Image.open(mockup_file) as pil_img:
                        corners = detect_corner_points(pil_img.convert("RGBA"))
...
/* === NEW: Mockup Thumbnail Loader === */
.mockup-thumb-container {
  position: relative;
  width: 100%;
  height: 225px; /* Must match .mockup-thumb-img height */
...
.theme-dark .mockup-thumb-container {
...
.theme-dark .mockup-spinner {
  border-color: rgba(255, 255, 255, 0.2);
  border-top-color: #fff;
}

.mockup-thumb-img.is-loading {
  visibility: hidden;
}
.mockup-thumb-img.is-loaded {
  visibility: visible;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* --------- [ EL4. Mockup Thumbnail Loaders ] --------- */
  document.querySelectorAll('.mockup-thumb-img').forEach(img => {
    img.classList.add('is-loading');
    img.addEventListener('load', () => {
        img.classList.remove('is-loading');
        img.classList.add('is-loaded');
        const container = img.closest('.mockup-thumb-container');
        if (container) {
            const spinner = container.querySelector('.mockup-spinner');
    ...
    img.addEventListener('error', () => {
        ...
        const container = img.closest('.mockup-thumb-container');
        if (container) {
            container.innerHTML = '<div class="missing-img">Load Error</div>';
        }
    });
  });
});
...
<div class="gallery-section">
    ...
    <div class="artwork-grid">
      ...
      <div class="gallery-card" data-filename="{{ art.filename }}">
        <div class="card-thumb">
          {# This url_for call was already correct #}
  ...
  {% if processed_artworks %}
    ...
    <div class="artwork-grid">
      ...
      <div class="gallery-card" data-filename="{{ art.filename }}">
        <div class="card-thumb">
          {# FIX: Corrected url_for to use a single 'path' argument instead of 'seo_folder' and 'filename' #}
          <img class="card-img-top"
               src="{{ url_for('artwork.processed_image', path=art.seo_folder + '/' + art.thumb) }}"
               alt="{{ art.title }}">
        </div>
        <span class="status-icon"></span>
        <div class="card-details">
  ...
  {% if finalised_artworks %}
    ...
    <div class="artwork-grid">
      ...
      <div class="gallery-card" data-filename="{{ art.filename }}">
        <div class="card-thumb">
          {# FIX: Corrected url_for to use a single 'path' argument #}
          <img class="card-img-top"
               src="{{ url_for('artwork.finalised_image', path=art.seo_folder + '/' + art.thumb) }}"
               alt="{{ art.title }}">
        </div>
        <span class="status-icon"></span>
        <div class="card-details">
...
{# Edit and finalise a single artwork listing with preview and metadata fields.
   Updated 2025-07-24: responsive thumbnails & cache busting #}
{% extends "main.html" %}
{% block title %}Edit Listing{% endblock %}
{% block content %}
<style>
  /* Styles moved from inline to classes for best practices */
  .action-form {
    width: 100%;
  }
  .form-col {
...
<div class="review-artwork-grid row">
  <div class="col col-6 mockup-col">
    <div class="main-thumb">
      ...
      <a href="#" 
          ...
          data-img="{{ url_for('artwork.processed_image', path=seo_folder + '/' + seo_folder + '.jpg') }}?t={{ cache_ts }}">
          <img src="{{ url_for('artwork.processed_image', path=seo_folder + '/' + seo_folder + '-THUMB.jpg') }}?t={{ cache_ts }}" class="main-artwork-thumb" alt="Main artwork thumbnail for {{ seo_folder }}">
      </a>
      ...
      <div class="mockup-preview-grid">
        {% for m in mockups %}
          <div class="mockup-card">
            ...
            {# ** NEW: Container for thumbnail and spinner ** #}
            <div class="mockup-thumb-container">
              <div class="mockup-spinner"></div>
              {% if m.exists %}
                {# FIX: Corrected url_for to use a single 'path' argument #}
                <a href="{{ url_for('artwork.processed_image', path=seo_folder + '/' + m.path.name) }}?t={{ cache_ts }}"
                  class="mockup-img-link"
                  data-img="{{ url_for('artwork.processed_image', path=seo_folder + '/' + m.path.name) }}?t={{ cache_ts }}">
                  
                  {% if m.thumb_exists %}
                    {# FIX: Corrected url_for to use a single 'path' argument #}
                    <img src="{{ url_for('artwork.processed_image', path=seo_folder + '/thumbnails/' + m.path.name) }}?t={{ cache_ts }}" class="mockup-thumb-img" alt="Mockup preview {{ loop.index }}">
                  {% else %}
                    {# FIX: Corrected url_for to use a single 'path' argument #}
                    <img src="{{ url_for('artwork.processed_image', path=seo_folder + '/' + m.path.name) }}?t={{ cache_ts }}" class="mockup-thumb-img" alt="Mockup preview {{ loop.index }}">
                  {% endif %}

                </a>
              {% else %}
                <img src="{{ url_for('static', filename='img/default-mockup.jpg') }}" class="mockup-thumb-img is-loaded" alt="Default mockup placeholder">
              {% endif %}
            </div>

            {# FIX: Changed url_for from 'artwork.review_swap_mockup' to the correct 'mockup.review_swap_mockup' #}
            <form role="form" method="post" action="{{ url_for('mockup.review_swap_mockup', seo_folder=seo_folder, slot_idx=m.index) }}" class="swap-form">
  ...
  <div class="col col-6 edit-listing-col">
    ...
    <div class="edit-actions-col">
      ...
      {% if not finalised %}
        {# This action seems to be missing from the provided routes, but leaving the form structure #}
      ...
      {% endif %}
      {# This action seems to be missing from the provided routes, but leaving the form structure #}
      <form role='form' method="POST" action="#" class="action-form analyze-form">
        <select name="provider" class="form-select form-select-sm mb-2" aria-label="Select AI provider for re-analysis">
    ...
    {% if openai_analysis %}
      <div class="openai-details">
        ...
        <table class="openai-analysis-table">
          <thead>
            <tr><th colspan="2">OpenAI Analysis Details</th></tr>
          </thead>
          <tbody>
            <tr><th scope="row">Original File</th><td>{{ info.original_file }}</td></tr>
            <tr><th scope="row">Optimized File</th><td>{{ info.optimized_file }}</td></tr>
            <tr><th scope="row">Size</th><td>{{ info.size_mb }} MB ({{ info.size_bytes }} bytes)</td></tr>
            <tr><th scope="row">Dimensions</th><td>{{ info.dimensions }}</td></tr>
            <tr><th scope="row">Time Sent</th><td>{{ info.time_sent }}</td></tr>
            <tr><th scope="row">Time Responded</th><td>{{ info.time_responded }}</td></tr>
            <tr><th scope="row">Duration</th><td>{{ info.duration_sec }} s</td></tr>
            <tr><th scope="row">Status</th><td>{{ info.status }}</td></tr>
            {% if info.api_response %}<tr><th scope="row">API Response</th><td>{{ info.api_response }}</td></tr>{% endif %}
            {% if info.naming_method %}<tr><th scope="row">Naming Method</th><td>{{ info.naming_method }}{% if info.used_fallback_naming %} (fallback){% endif %}</td></tr>{% endif %}
          </tbody>
        </table>
        {% endfor %}
      </div>
