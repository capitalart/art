# FULL CODE STACK (MON-28-JULY-2025-12-40-PM)
...
:root {
  ...
  --color-text: #111111;
  --color-card-bg: #bbbbbb;
  --color-header-border: #eeeeee;
  --color-footer-bg: #bbbbbb;
  ...
  <div class="review-artwork-grid row">
    <div class="col col-6 mockup-col">
      <div class="main-thumb">
          {# *** CORRECTED LOGIC FOR IMAGE URLS *** #}
          ...
          <a
            ...
            data-img="{{ analyse_img_url }}?t={{ cache_ts }}"
          >
          <img
            src="{{ thumb_img_url }}?t={{ cache_ts }}"
        ...
        {% for m in mockups %}
          <div class="mockup-card" id="mockup-card-{{ m.index }}">
            {% if m.exists and m.thumb_exists %}
              {# *** CORRECTED LOGIC FOR MOCKUP URLS *** #}
              {% if is_locked_in_vault %}
                {% set thumb_url = url_for('artwork.serve_mockup_thumb', filepath=seo_folder ~ '/THUMBS/' ~ m.thumb.name) %}
                {% set full_url = url_for('artwork.locked_image', filename=seo_folder ~ '/' ~ m.path.name) %}
              {% elif finalised %}
                {% set thumb_url = url_for('artwork.serve_mockup_thumb', filepath=seo_folder ~ '/THUMBS/' ~ m.thumb.name) %}
                {% set full_url = url_for('artwork.finalised_image', filename=seo_folder ~ '/' ~ m.path.name) %}
              {% else %}
                {% set thumb_url = url_for('artwork.serve_mockup_thumb', filepath=seo_folder ~ '/THUMBS/' ~ m.thumb.name) %}
                {% set full_url = url_for('artwork.processed_image', filename=seo_folder ~ '/' ~ m.path.name) %}
              {% endif %}
              <a
                href="{{ full_url ~ '?t=' ~ cache_ts }}"
                class="mockup-img-link"
                id="mockup-link-{{ m.index }}"
                data-img="{{ full_url ~ '?t=' ~ cache_ts }}"
              >
                <img
                  id="mockup-img-{{ m.index }}"
                  src="{{ thumb_url ~ '?t=' ~ cache_ts }}"
                  data-fallback="{{ full_url ~ '?t=' ~ cache_ts }}"
            ...
            {% else %}
              <img
                src="{{ url_for('static', filename='img/default-mockup.jpg') }}"
                class="mockup-thumb-img"
    ...
    </div> <div class="col col-6 edit-listing-col">
...
<script>
  // Pass necessary info from Flask/Jinja2 to the JavaScript file
  window.EDIT_INFO = {
...
<form method="post" action="{{ url_for('exports.run_sellbrite_csv_export') }}">
...
<div class="container">
    ...
    <div class="exports-special-grid">
        <div class="status-and-actions">
            ...
            <hr>
            <a href="{{ url_for('exports.sellbrite_csv_exports') }}" class="btn btn-secondary wide-btn">View Legacy CSV Exports</a>
        </div>
