# FULL CODE STACK (SUN-27-JULY-2025-11-50-PM)
    ...
    <div class="col col-6 mockup-col">
      <div class="main-thumb">
          <a
            ...
            data-img="{{ url_for('artwork.processed_image', seo_folder=seo_folder, filename=seo_folder ~ '-ANALYSE.jpg') }}?t={{ cache_ts }}"
          >
          <img
          ...
          {% for info in entries %}
            <table class="openai-analysis-table">
              ...
              <tbody>
                ...
                {% if info.api_response %}<th>API Response</th><td>{{ info.api_response }}</td>{% endif %}
                {% if info.naming_method %}<th>Naming Method</th><td>{{ info.naming_method }}{% if info.used_fallback_naming %} (fallback){% endif %}</td>{% endif %}
              </tbody>
            </table>
          {% endfor %}
        </div>
