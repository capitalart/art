...
def generate_ai_listing(system_prompt: str, image_path: Path, aspect: str, assigned_sku: str, feedback: str | None = None) -> tuple[dict, bool, str]:
    """Call OpenAI Vision with an optimized image and return parsed listing.

    ``assigned_sku`` is injected into the prompt so the AI never invents a SKU.
    """
    try:
    ...
    messages = [{"role": "system", "content": full_prompt}, {"role": "user", "content": user_content}]
    if feedback:
...
def analyze_single(image_path: Path, system_prompt: str, feedback_text: str | None, statuses: list):
    ...
    try:
        ...
        try:
            ai_listing, was_json, raw_response = generate_ai_listing(
                system_prompt,
                opt_img,
                aspect,
                assigned_sku,
            ...
            logger.error(traceback.format_exc())
            log_entry.update({"status": "fail", "error": str(e)})
