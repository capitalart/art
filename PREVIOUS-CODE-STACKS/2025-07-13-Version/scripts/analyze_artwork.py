...
``art_narrator/logs/analyze-artwork-YYYY-MM-DD-HHMM.log``.
...
def assemble_gdws_description(aspect: str) -> str:
    """Assemble final description from gdws_content folders."""
    aspect_path = BASE_DIR / "gdws_content" / aspect
    if not aspect_path.exists():
        logger.warning("GDWS content for %s not found", aspect)
        return ""
    blocks = []
    for p_folder in sorted([p for p in aspect_path.iterdir() if p.is_dir()]):
        variations = list(p_folder.glob("*.json"))
        if not variations:
            continue
        ...
        try:
            ...
            logger.error("Failed loading %s: %s", chosen, exc)
            continue
